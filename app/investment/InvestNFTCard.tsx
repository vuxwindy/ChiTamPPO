...
function floorFragment(num, digits) {
...
//   const { ppoPackageAddress } = useContractAddress();
  ...
  const isActive = useMemo(() => {
    return (
      dayjs().isAfter(dayjs(Number(nft.startTime) * 1000)) &&
      dayjs().isBefore(dayjs(Number(nft.endTime) * 1000))
    );
  }, [nft.startTime, nft.endTime]);

  const lockPeriod = useMemo(() => {
    return dayjs(Number(nft.endTime) * 1000).diff(
      dayjs(Number(nft.startTime) * 1000),
      "day"
    );
  }, [nft.startTime, nft.endTime]);

  const mintStake = useMemo(
    () => ethers.formatUnits(nft.buyAmountBNB?.toString() || "0", 18),
    [nft.buyAmountBNB]
  );

  const ppoPerDay = useMemo(
    () =>
      floorFragment(
        ethers.formatUnits(nft.ppoPerDay?.toString() || "0", 18),
        5
      ),
    [nft.ppoPerDay]
  );

  const pendingPPO = useMemo(
    () =>
      floorFragment(
        ethers.formatUnits(nft.pendingPPO?.toString() || "0", 18),
  ...
  }, [address, chainId, nft.tokenId, onClaimed]);
  ...
  return (
    <div className="pool-card p-[16px] md:p-[30px]">
      <div className="pool-header">
        ...
        <div className={`pool-status ${isActive ? "active" : "inactive"}`}>
